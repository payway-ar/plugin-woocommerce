...
function decidir_init_gateway_class()
{
    ...
    class WC_Decidir_Gateway extends WC_Payment_Gateway
    {
        ...
        public function __construct()
        {
            ...
            $this->testmode             = true === $this->get_option('testmode');
            $this->usecybersource       = true === $this->get_option('usecybersource');
            ...
            add_action('woocommerce_api_decidir', array(
                $this,
                'webhook',
            ));
    ...
    <?php
            ...
            if ($this->settings['testmode'] == false) {
...
<?php
        ...
        public function payment_scripts()
        {
            ...
            wp_register_script('woocommerce_decidir', plugins_url('assets/js/card.js', __FILE__), array(
                'jquery',
                'decidir_js',
            ));
        ...
        public function process_payment($order_id)
        {
            ...
            foreach ($order->get_items() as $item_key => $item):
                ...
                $items[] = ["csitproductcode" => $product_sku, "csitproductdescription" => $item_name, "csitproductname" => $item_description, "csitproductsku" => $product_sku, "csittotalamount" => $product_price * $quantity, "csitquantity" => $quantity, "csitunitprice" => $product_price];
            ...
            if ($this->settings['usecybersource'] == "yes") {
                ...
                $cybersource = new Decidir\Cybersource\Retail($cs_data, // Datos de la operación
                    $cs_products, // Datos de los productos
                );

                $connector->payment()
                    ->setCybersource($cybersource->getData());
            ...
            $data                        = array(
                ...
                "customer"            => array(
                    "id"         => $dec_CardFirstName . '_' . $dec_CardLastName,
            ...
            if ($status == 'approved') {
                ...
                $order->add_order_note(sprintf("Detalle pago: '%s'", $response->getId() . ' - ' . $status));
                ...
                $order->add_order_note(sprintf("Detalle pago: '%s'", $status));
                $order->add_order_note(sprintf("Detalle error: '%s'", $details));
