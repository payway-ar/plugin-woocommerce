...
$PAGE_KEY_NAME = 'decidir_admin_banks';
	...
	<?php else: ?>
		<?php if (isset( $_POST['action'] )): ?>
			<?php if ( $_POST['action'] === $ACTION_NAME_NEW ): ?>
				<?php check_admin_referer( 'decidir-create-edit-bank', '_wpnonce_decidir-create-edit-bank' ); ?>
				<?php
					$errors = new WP_Error();
		...
		<?php elseif ( $ACTION_NAME_NEW === $params['action']
			|| $ACTION_NAME_EDIT === $params['action'] ): ?>
				...
				<form method="post" name="<?php echo $params['action'] ?>" id="<?php echo $params['action'] ?>" class="validate" novalidate="novalidate">
					...
					<?php endif; ?>
					<?php wp_nonce_field( 'decidir-create-edit-bank', '_wpnonce_decidir-create-edit-bank' ); ?>
					<?php
		...
		<?php elseif ( $ACTION_NAME_DELETE === $params['action'] ): ?>
			...
			<?php wp_verify_nonce( '_wpnonce_decidir-delete-bank-' . $params['bank_id'] ); ?>
