...
$PAGE_KEY_NAME = 'payway_admin_cards';
...
$title = $is_creation
	...
	: esc_html__('Cards', 'wc-gateway-payway');
?>
<div class="wrap">
	...
	<?php if ( ! isset($params['action']) ): ?>
		...
		<a href="<?php echo esc_url( admin_url( 'admin.php?page=' . $PAGE_KEY_NAME . '&action=' . $ACTION_NAME_NEW ) ); ?>" class="page-title-action">
			<?php echo __( 'Add New', 'wc-gateway-payway' ); ?>
		</a>
		<?php echo $this->get_table(); ?>
	<?php else: ?>
		<?php if (isset( $_POST['action'] )): ?>
			<?php if ( $_POST['action'] === $ACTION_NAME_NEW ): ?>
				<?php check_admin_referer( 'payway-create-edit-card', '_wpnonce_payway-create-edit-card' ); ?>
				<?php
					$errors = new WP_Error();
					...
					if ( isset( $_POST['card_name'] ) && $_POST['card_name'] != '' ) {
						...
						$errors->add( 'card_name', __('Name is a required field', 'wc-gateway-payway') );
					}

					if ( isset( $_POST['id_sps'] ) && $_POST['id_sps'] != '' ) {
						$data['id_sps'] = sanitize_text_field( $_POST['id_sps'] );
					} else {
						$errors->add( 'id_sps', __('ID SPS is a required field', 'wc-gateway-payway') );
					}
					if ( isset( $_POST['id_nps'] ) && $_POST['id_nps'] != '' ) {
						$data['id_nps'] = sanitize_text_field( $_POST['id_nps'] );
					} else {
						$errors->add( 'id_nps', __('ID NPS is a required field', 'wc-gateway-payway') );
					}
				?>
				<?php if ( ! $errors->has_errors() ): ?>
					...
					<?php if ( $result ): ?>
						...
						<?php die(); ?>
					<?php else: ?>
						<p><?php echo __('Some things happened during Card creation', 'wc-gateway-payway'); ?></p>
			...
			<?php elseif ( $_POST['action'] === $ACTION_NAME_EDIT ): ?>
				...
				<?php if ( $result ): ?>
					...
					<?php die(); ?>
				<?php else: ?>
					<p><?php echo __('Some things happened while updating the Card', 'wc-gateway-payway'); ?></p>
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=' . $PAGE_KEY_NAME ) ); ?>"><?php echo __('Go back', 'wc-gateway-payway'); ?></a>
				<?php endif; ?>
			<?php else: ?>
		...
		<?php elseif ( $ACTION_NAME_NEW === $params['action']
			|| $ACTION_NAME_EDIT === $params['action'] ): ?>
				...
				<form method="post" name="<?php echo $params['action'] ?>" id="<?php echo $params['action'] ?>" class="validate" novalidate="novalidate">
					...
					<?php endif; ?>
					<?php wp_nonce_field( 'payway-create-edit-card', '_wpnonce_payway-create-edit-card' ); ?>
					<?php
					...
					<table class="form-table" role="presentation">
						<tr class="form-field form-required">
							<th scope="row"><label for="card_name">
								<?php _e( 'Card Name', 'wc-gateway-payway' ); ?> <span class="description"><?php _e( '(required)' ); ?></span></label>
							</th>
							<td>
						...
						<tr class="form-field form-required">
							<th scope="row"><label for="id_sps">
								<?php _e( 'ID SPS', 'wc-gateway-payway' ); ?> <span class="description"><?php _e( '(required)' ); ?></span></label>
							</th>
							<td>
						...
						<tr class="form-field form-required">
							<th scope="row"><label for="id_nps">
								<?php _e( 'ID NPS', 'wc-gateway-payway' ); ?> <span class="description"><?php _e( '(required)' ); ?></span></label>
							</th>
							<td>
		...
		<?php elseif ( $ACTION_NAME_DELETE === $params['action'] ): ?>
			...
			<?php wp_verify_nonce( '_wpnonce_payway-delete-card-' . $params['card_id'] ); ?>
